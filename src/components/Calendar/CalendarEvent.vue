<template>
  ...
  <div
    ...
    v-bind="$attrs"
    :data-event-id="props.event?.id || props.event?.name"
    :class="[
...
const activeEventId = inject('activeEventId', '')
...
const updatedEvent = reactive({
  ...props.event,
  date: props.event.date || props.event.fromDate,
})
...
watch(
  ...
  (newVal) => {
    ...
    updatedEvent.date = newVal.date || newVal.fromDate
...
const setEventStyles = computed(() => {
  ...
  // Boost z-index for the currently active event so it stays clickable
  // while the popup is open (prevents overlapping events from stealing clicks)
  if (
    activeEventId.value &&
    activeEventId.value === (props.event?.id || props.event?.name)
  ) {
    zIndex = Math.max(zIndex, 150)
  }
...
function handleRepositionMouseDown(e) {
  ...
  function mouseup(e) {
    ...
    const dateChanged = updatedEvent.date && calendarEvent.value.date !== updatedEvent.date
    if (dateChanged) {
      isEventUpdated.value = true
    }
...
function handleHorizontalMovement(clientX, rect) {
  // Use props.date instead of parentNode attribute to avoid
  // Invalid Date when data-date-attr is missing or malformed.
